...
class Client {
  ...
  orders = {
    ...
    createGuest: (input: CreateOrderInput) =>
      HttpClient.post<Order>(API_ENDPOINTS.ORDERS_GUEST, input),
  ...
  users = {
    me: () => HttpClient.get<User>(`${API_ENDPOINTS.USERS_ME}?timestamp=${new Date().getTime()}`),
    update: (user: UpdateUserInput) =>
      HttpClient.put<User>(`${API_ENDPOINTS.USERS}/${user.id}?timestamp=${new Date().getTime()}`, user),
